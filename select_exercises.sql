...
SELECT * FROM `albums`; #31 in ID, i.e. 31 rows 
SELECT COUNT(DISTINCT artist) FROM `albums`; #23 rows affected
-- 23 shows at the bottom next to no errors, but there must be a way to make it appear more directly

SELECT MIN(release_date) FROM `albums`; #1967 is the oldest release date
SELECT MAX(release_date) FROM `albums`; #2011 is FRESH
...
SELECT name FROM albums WHERE artist = 'Pink Floyd'; #I need to work on understanding when to use apostrophes or ticks
#Nevertheless, the answer is The Dark Side of the Moon and The Wall

SELECT release_date FROM albums WHERE name = "Sgt. Pepper's Lonely Hearts Club Band"; #1967 That went much more smoothly
